...
public abstract class I2CSensor {
    ...
    private void connectToDevice() {
        ...
        command = ByteBuffer.allocateDirect(registerSize);
    ...
    /**
     * This method tries to write one single byte to particular register
     *
     * @param register Register to write
     * @param byteToWrite Byte to be written
     */
    public void writeOneByte(int register, byte byteToWrite) {
        command.clear();
        command.put(byteToWrite);
        command.rewind();
        try {
            myDevice.write(register, registerSize, command);
        } catch (IOException e) {
    ...
    /**
     * This method reads one byte from a specified register address. The method
     ...
     */
    public byte readOneByte(int register) {
        byteToRead.clear();
        int result = -1;
        try {
            result = myDevice.read(register, registerSize, byteToRead);
        } catch (IOException e) {

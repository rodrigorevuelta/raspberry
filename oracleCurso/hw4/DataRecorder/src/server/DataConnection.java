    ...
    while (running) {
      try {
        ...
        if (command.compareTo(READ_DATA) == 0) {
          ...
          printMessage("sending: " + dataSource.getRecordCount() + " records", DATA);
          // Send the number of message strings first
          output.write(Integer.toString(dataSource.getRecordCount()));
          output.newLine();
          // Send all the messages
          for (int i = 1; i <= dataSource.getRecordCount(); i++) {
            try {
              output.write(dataSource.getRecord(i));
            } catch (InvalidRecordIDException ex) {
              System.out.println("DataConnection: run: " + ex);
            }
            output.newLine();
          }
          output.flush();
